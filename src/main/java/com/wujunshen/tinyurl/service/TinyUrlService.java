...
public class TinyUrlService {
    ...
    public String getOriginUrl(String tinyUrl) {
        // 先从缓存中获取
        String originUrl = myRedisTemplate.opsForValue().get(tinyUrl);
        ...
        if (urlMapping != null) {
            ...
            // 放入缓存,防止缓存穿透
            myRedisTemplate.expire(tinyUrl, EXPIRED_TIME, TimeUnit.HOURS);
        }
    ...
    public TinyUrlData genTinyUrl(GenTinyUrlRequest request) {
        ...
        // 放入缓存,防止缓存穿透
        myRedisTemplate.expire(customTinyUrl, EXPIRED_TIME, TimeUnit.HOURS);
